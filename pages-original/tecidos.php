<?php
$title = " Tecidos lisos para Personalização - Estamparia Digital em Metro Corrido ";
$description = "Tecidos lisos para Personalização - Estamparia Digital em Metro Corrido ";
include("partial/header.php") ?>
    <div class="row small-up-1 medium-up-2 large-up-2">
        <div class="column">
            <h1 class="titulo">Tecidos lisos para Personalização</h1>
                <h2 class="subheader titulo">
                    Estamparia Digital em Metro Corrido
                </h2>
            <p class="subheader">Solicite agora um orçamento !</p>

                <p class="subheader">
                    Estamos aguardando sua ligação para orientá-lo a definir qual o melhor tecido para fabricar seu produto. Escolha as estampas do nosso acervo ou envie sua ARTE e PERSONALIZE seu tecido. Você pode comprar uma amostra no tamanho 15cm x 15cm ou 70cm x 100cm para aprovação de cores e sentir o toque do tecido.<br>Seja nosso parceiro(a) e REVENDA tecidos PERSONALIZADOS !!!<br>
                    Envie as imagens em : .TIFF - RGB - 300 DPI - IMAGEM ACHATADA - RAPPORT RETO NOS QUATRO LADOS.
                </p>
        </div>
    <div class="column">
        <img src="images/calandra-para-web.jpg" alt="Rossina estamparia DIgital" class="">
    </div>
</div>
<div id="tecidoA"></div>
<br>
<hr class="hr-linha">
<div class="row">
    <h5 class="text-center titulo">
        Tecidos - Bases Poliéster
    </h5>
</div>
<hr class="hr-linha">
<div class="row small-up-1 medium-up-2 large-up-3">
        <div class="column">
            <img src="images/tecidos/tecido-amostras/maui-plus-160.jpg" width="500">
        </div>
        <div class="column">
            <h3 class="titulo">MicroFibra Pesada</h3>
            <p class="subheader">
                100% poliéster, 1,60 larg - 160gr/m2
            </p>
            <h5 class="titulo">
                Utilização
            </h5>
            <p class="subheader">
                Acessórios, agasalhos, bermudas, bolsas, detalhes,<br>
                jaquetas leves, macaquinhos, parkas, shorts, saias.
            </p>
        </div>
    <div class="column">
        <h3 class="titulo">
            Atributos
        </h3>
        <p class="subheader">
            Uma evolução do nosso já conhecido maui, porém mais estruturado e com largura de 1,60m.
            Levemente mais pesado que o antigo da linha maui.
        </p>
        <h5 class="titulo">
            Como lavar
        </h5>
        <img src="images/tecidos/tecido-amostras/como-lavar-micro-fibra-pesada.png">
    </div>
</div>

<hr class="hr-linha">

<div class="row small-up-1 medium-up-2 large-up-3">
        <div class="column">
            <img src="images/tecidos/tecido-amostras/techno-light-160.jpg">
        </div>
        <div class="column">
            <h3 class="titulo">MicroFibra Leve</h3>
            <p class="subheader">
                100% poliéster, 1,60 larg - 108gr/m2
            </p>
            <h5 class="titulo">
                Utilização
            </h5>
            <p class="subheader">
                colocar utilização
            </p>
            <h5 class="titulo">
                Atributos
            </h5>
            <p class="subheader">
                colocar atributos
            </p>
            <h5 class="titulo">
                Como lavar
            </h5>
            <img src="images/tecidos/tecido-amostras/como-lavar-micro-fibra-leve.png">
        </div>
        <div class="column">
            <p class="subheader">
                Vídeo - Plotter imprimindo colorido chapado no papel para transferência em tecidos.
            </p>
            <a class="video" href="../embed/video/mp4/plotter-dx7-subli-corrido-listrado-diagonal.mp4">
                <img title="Seu vídeo esta sendo carregado favor aguardar ......"
                 class="thumbnail float-right" src="../embed/video/imagem-play/print5.jpg"/>
            </a>
        </div>
</div>
<hr class="hr-linha">
<div class="row small-up-1 medium-up-2 large-up-3">
        <div class="column">
            <img src="images/tecidos/tecido-amostras/gloss-light.jpg">
        </div>
        <div class="column">
            <h3 class="titulo">Gloss Light</h3>
            <p class="subheader">
                100% poliéster, 1,50 larg - 80gr/m2
            </p>
            <h5 class="titulo">
                Utilização
            </h5>
            <p class="subheader">
                Blusas, Camisas, Vestidos.
            </p>
            <h5 class="titulo">
                Atributos
            </h5>
            <p class="subheader">
                Tecido plano, tinto em peça, construção cetim,
                formato por fios lisos de poliéster de titulagem fina.
            </p>
            <h5 class="titulo">
                Como lavar
            </h5>
            <img src="images/tecidos/tecido-amostras/como-lavar-gloss-light.png">
        </div>
        <div class="column">
            <p class="subheader">
                Vídeo - Plotter imprimindo colorido chapado no papel para transferência em tecidos.
            </p>
            <a class="video" href="../embed/video/mp4/plotter-dx5.5-subli-corrido-colorido.mp4">
                <img title="Seu vídeo esta sendo carregado favor aguardar ......"
                    class="thumbnail float-right" src="../embed/video/imagem-play/print.jpg"/>
            </a>
        </div>
</div>
<hr class="hr-linha">
<div class="row small-up-1 medium-up-2 large-up-3">
        <div class="column">
            <img src="images/tecidos/tecido-amostras/crepe-de-chine.jpg">
        </div>
        <div class="column">
            <h3 class="titulo">Crepe de Chine</h3>
            <p class="subheader">
                100% poliéster, 1,50 larg - 80gr/m2
            </p>
            <h5 class="titulo">
                Utilização
            </h5>
            <p class="subheader">
                Blusas, camisas, shorts, saias, vetidos
            </p>
            <h5 class="titulo">
                Atributos
            </h5>
            <p class="subheader">
                Com a forte tendência dos crepes, estamos lançando
                uma nova base com fio mais fino deixando com o toque
                que se assemelha ao crepe de chine de seda pura
            </p>
            <h5 class="titulo">
                Como lavar
            </h5>
            <img src="images/tecidos/tecido-amostras/como-lavar-crepe-de-chine.png">
        </div>
        <div class="column">
            <p class="subheader">
                Vídeo - Plotter imprimindo chapado preto no papel para transferência em tecidos.
            </p>
            <a class="video" href="../embed/video/mp4/plotter-dx5.5-subli-corrido-chapado-preto.mp4">
                <img title="Seu vídeo esta sendo carregado favor aguardar ......"
                 class="thumbnail float-right" src="../embed/video/imagem-play/chapado-preto.jpg"/>
            </a>
            <div id="tecidoE"></div>
        </div>
</div>
<hr class="hr-linha">
<div class="row small-up-1 medium-up-2 large-up-3">
        <div class="column">
            <img src="images/tecidos/tecido-amostras/hi-multi-chiffon-classic.jpg"/>
        </div>
        <div class="column">
            <h3 class="titulo">Chiffon Classic</h3>
            <p class="subheader">
                100% poliéster, 1,50 larg - 80gr/m2
            </p>
            <h5 class="titulo">
                Utilização
            </h5>
            <p class="subheader">
                colocar utilização
            </p>
            <h5 class="titulo">
                Atributos
            </h5>
            <p class="subheader">
                Artigo com construção em tela, produzido<br>
                filamentos de poliéster de alta torção e tinto em peça
            </p>
            <h5 class="titulo">
                Como lavar
            </h5>
            <img src="images/tecidos/tecido-amostras/como-lavar-chiffon-classic.png">
        </div>
        <div class="column">
            <p class="subheader">
                Vídeo - Calandra rolo a rolo peça pronta.
            </p>
            <a class="video" href="../embed/video/mp4/calandra-rolo-peca-pronta.mp4">
                <img title="Seu vídeo esta sendo carregado favor aguardar ......"
                 class="thumbnail float-right" src="../embed/video/imagem-play/print10.jpg"/>
            </a>
            <div id="tecidoF"></div>
        </div>
</div>
<hr class="hr-linha">
    <div class="row small-up-1 medium-up-2 large-up-3">
        <div class="column">
            <img src="images/tecidos/tecido-amostras/oxford-stretch-tinto.jpg"/>
        </div>
        <div class="column">
            <h3 class="titulo">Oxford Strech</h3>
            <p class="subheader">
                100% poliéster, 1,50 larg - 160gr/m2
            </p>
            <h5 class="titulo">
                Utilização
            </h5>
            <p class="subheader">
                Acessórios, alfaiatarias, agasalhos, bermudas, fitness,
                blazers, costumes, calças, casacos, linha cama
                 e colchas, sapatos, saias, ternos,
                vestidos.
            </p>
            <h5 class="titulo">
                Atributos
            </h5>
            <br>
            <p class="subheader">
                Alta durabilidade, não amassa, grande<br>
                vesatilidade, boa solidez de cores.
            </p>
            <h5 class="titulo">
                Como lavar
            </h5>
            <img src="images/tecidos/tecido-amostras/como-lavar-orford-strech.png">
        </div>
        <div class="column">
            <p class="subheader">
                Vídeo - Camisas - camisetas full print.
            </p>
            <a class="video" href="../embed/video/mp4/camisetas-camisas-full-print.mp4">
                <img title="Seu vídeo esta sendo carregado favor aguardar ......"
                 class="thumbnail float-right" src="../embed/video/imagem-play/print8.jpg"/>
            </a>
            <div id="tecidoG"></div>
        </div>
    </div>
<hr class="hr-linha">
    <div class="row small-up-1 medium-up-2 large-up-3">
        <div class="column">
            <img src="images/tecidos/tecido-amostras/malha-newprene.jpg"/>
        </div>
        <div class="column">
            <h3 class="titulo">Malha Newprene</h3>
            <p class="subheader">
                90% poliéster/4%elastano, 1,70 larg - rend. 1,90 m/k - 380gr/m2
            </p>
            <h5 class="titulo">
                Utilização
            </h5>
            <p class="subheader">
                Acessórios, boleros, bermudas fitness, calças, coletes<br>
                casaquetos, casacos, detalhes, jaquetas leves, legging,<br>
                saias, tops, vestidos.
            </p>
        </div>
        <div class="column">
            <h5 class="titulo">
                Atributos
            </h5>
            <p class="subheader">
                Malha dupla compacta, bom isolamento térmico, toque<br>
                macio, boa flexibilidade. Ideal para segmento feminino,<br>
                masculino e esportivo
            </p>
            <div id="tecidoH"></div>
            <h5 class="titulo">
                Como lavar
            </h5>
            <img src="images/tecidos/tecido-amostras/como-lavar-malha-newprene.png">
        </div>
    </div>
    <hr class="hr-linha">
    <div class="row small-up-1 medium-up-2 large-up-3">
        <div class="column">
            <img src="images/tecidos/tecido-amostras/ergonomic-flex.jpg"/>
        </div>
        <div class="column">
            <h3 class="titulo">Ergonomic Flex</h3>
            <p class="subheader">
                92% poliéster/8%elastano, 1,50 larg - 124gr/m2
            </p>
            <h5 class="titulo">
                Utilização
            </h5>
            <p class="subheader">
                Bermudas fitness, shorts.
            </p>
        </div>
        <div class="column">

        </div>
    </div>
</div>
<?php include("partial/footer.php") ?>